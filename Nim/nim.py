...
def Get_Move_From_Player():
    while True:
        Display_Heaps()
        heap = raw_input("It's {0}'s turn. Which heap do you want to take from? : ".format(players[player]))
        if not heap in heaps:
            print "There is no heap called {0}".format(heap)
        else:
            how_many = input("How many would you like to take from heap {0}? : ".format(heap))
            if how_many > heaps[heap]:
                print "There aren't {0} pieces in heap {1}".format(how_many, heap)
            else:
                return heap, how_many

player1 = raw_input("Enter Player One's Name: ")
player2 = raw_input("Enter Player Two's Name: ")
